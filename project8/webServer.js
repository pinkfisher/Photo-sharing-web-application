...
app.get('/user/list', function (request, response) {
    ...
    User.find({},function(err,usersDB) {
        ...
        for (var i = 0; i < users.length; i++){
            ...
            userObj.location = users[i].location;
            userObj.description = users[i].description;
            userObj.occupation = users[i].occupation;
            userObj.activity = users[i].activity;
            userObj.recent_photo = users[i].recent_photo;
            userList = userList.concat([userObj]);
        }
...
app.get('/user/:id', function (request, response) {
    ...
    User.findOne({_id:id},function(err,userDB){
        ...
        user.activity = userDetail.activity;
        user.recent_photo = userDetail.recent_photo;
...
app.post('/admin/login',function(request,response){
    ...
    User.findOne({login_name:login_name},function(err,userDB){
        ...
        userDB.activity = "User logged in";
        userDB.save();
...
app.post('/admin/logout',function(request,response){
    ...
    User.findOne({_id:request.session.user_id},function(err,user){
        ...
        if(user.length === 0){
            ...
            response.status(400).send("Can not find the user.");
            return;
        }
        user.activity = "User logged out";
        user.save();
    });
...
app.post('/user',function(request,response){
    ...
    User.findOne({login_name:newName},function(err,user){
        ...
        location:location,description:description,occupation:occupation,activity:activity},function(err,newUser){
            if(err){
                response.status(400).send(JSON.stringify(err));
                return;
            }
...
app.post('/deletePhoto/:photo_id',function(request,response){
    ...
    Photo.findOne({_id:photoId},function(err,photo){
        if(err){
            response.status(400).send("Delete photo failed.");
            return;
        }
        if (photo === null){
            console.log('Invalid photo id');
            response.status(400).send('Inalid photo id');
            return;
        }

        if (photo.user_id === request.session.user_id){
            console.log('You are not allowed to delete this photo.');
            response.status(400).send('You are not allowed to delete this photo.');
            return;
        }
...
app.post('/deleteComment/:photo_id',function(request,response){
    ...
    Photo.findOne({_id:photoId},function(err,photo){
        if(err){
            response.status(400).send("Delete comment failed.");
            return;
        }
        if(photo === null){
            console.log('Invalid photo id');
        ...
        if(comment.user._id !== request.session.user_id){
            ...
            response.status(400).send('You are not allowed to delete this comment.');
            return;
        }
        ...
        for (var i = 0; i < comments.length; i++){
            if (String(comments[i]._id) !== String(comment._id)){
                newComments = newComments.concat([comments[i]]);
            }
        }
...
app.post('/delete/:id',function(request,response){
    ...
    if (String(userId) !== String(request.session.user_id)){
        console.log('You are not eligible to delete the user.');
        response.status(400).send('You are not eligible to delete the user.');
        return;
    }
    ...
    Photo.find({},function(err,photos){
        ...
        for(var i = 0; i < photos.length; i++){
            ...
            comments = comments.filter(function(com){
                return String(com.user_id) !== String(userId);
            });
...
app.post('/like/:photo_id',function(request,response){
    ...
    Photo.findOne({_id:photoId},function(err,photo){
        ...
        if(photo === null){
            console.log("Can not find the photo");
            response.status(400).send("Can not find the photo");
        }
        var index = photo.likes_ids.indexOf(userId);
        if(index === -1){
            photo.like_count++;
            photo.likes_ids = photo.likes_ids.concat([userId]);
            photo.save();
            response.status(200).send("Like added.");
        }else{
            photo.like_count--;
